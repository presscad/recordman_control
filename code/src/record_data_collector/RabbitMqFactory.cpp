...
CRabbitMqFactory::CRabbitMqFactory(void)
{
...
void CRabbitMqFactory::SetConfigVariableHandle(CConfigVariableMgr* pObj)
{
...
//************************************
bool CRabbitMqFactory::ConnectRabbitMqServer(int nchannelid)
{
	bool bRet = true;
...
bool CRabbitMqFactory::LoginAndOpenChannel(int nchannelid)
{
	...
	try
	{
		...
		login_replay = amqp_login(
			...
			m_pConfigVariableObj->m_rabbit_mq_param.chusrname, 
			m_pConfigVariableObj->m_rabbit_mq_param.chpassword);
...
bool CRabbitMqFactory::find_operation_server_error(int nErrorNo, const char* pContext)
{
	if (nErrorNo < 0)
	{
...
bool CRabbitMqFactory::find_amqp_error(amqp_rpc_reply_t replay, const char* pContext)
{
