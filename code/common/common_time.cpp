/********************************************************************
	...
	filename: 	COMMON\common_time.cpp
	file path:	COMMON
	file base:	common_time
	file ext:	cpp
	author:		
...
*************************************************************/
CCommonTime::CCommonTime( CCommonTime& pXJTime )
...
*************************************************************/
CCommonTime::CCommonTime( SYSTIME& pSYSTime )
:m_iTimeZone(8)
{
...
*************************************************************/
CCommonTime::CCommonTime( time_t& pTTime )
:m_iTimeZone(8)
{
    //tm 结构
...
*************************************************************/
CCommonTime& CCommonTime::operator=( CCommonTime& pTime )
{
    if (this != &pTime)
    {
...
*************************************************************/
std::string CCommonTime::GetTimeString( int pType ) 
{
    mktime(&m_Time);
    string strDest;
    switch (pType)
...
*************************************************************/
bool CCommonTime::AssignTimeString( string pTimeStr, int pType )
{
    bool bResult(true);
    switch (pType)
    {
...
/*************************************************************
 ...
 返 回 值:	u_short
*************************************************************/
u_short CCommonTime::Year() const
{
...
/*************************************************************
 ...
 参    数:	pYear 输入的年
*************************************************************/
void CCommonTime::Year( u_short pYear )
{
...
/*************************************************************
 ...
 返 回 值:	u_short
*************************************************************/
u_short CCommonTime::Month() const
{
...
/*************************************************************
 ...
 参    数:	pMonth 输入的月
*************************************************************/
void CCommonTime::Month( u_short pMonth )
{
...
/*************************************************************
 ...
 参    数:	pDay 输入的日
*************************************************************/
void CCommonTime::Day( u_short pDay )
{
    m_Time.tm_mday = pDay;
}
...
/*************************************************************
 ...
 返 回 值:	u_short
*************************************************************/
u_short CCommonTime::Hour() const
{
...
/*************************************************************
 ...
 参    数:	pHour 输入的时
*************************************************************/
void CCommonTime::Hour( u_short pHour )
{
...
/*************************************************************
 ...
 返 回 值:	u_short
*************************************************************/
u_short CCommonTime::Minute() const
{
...
/*************************************************************
 ...
 参    数:	pMinute 输入的分
*************************************************************/
void CCommonTime::Minute( u_short pMinute )
{
...
/*************************************************************
 ...
 返 回 值:	u_short
*************************************************************/
u_short CCommonTime::Second() const
{
...
/*************************************************************
 ...
 参    数:	pSecond 输入的秒
*************************************************************/
void CCommonTime::Second( u_short pSecond )
{
...
/*************************************************************
 ...
 返 回 值:	double
*************************************************************/
double CCommonTime::MilliSecond() const
{
...
*************************************************************/
void CCommonTime::MilliSecond( double pMilliSecond )
{
...
/*************************************************************
 ...
 返 回 值:	int
*************************************************************/
int CCommonTime::TimeZone() const
{
    return m_iTimeZone;
}
...
/*************************************************************
 ...
 参    数:	val 输入的时区
*************************************************************/
void CCommonTime::TimeZone( int val )
{
...
*************************************************************/
double CCommonTime::DiffTime( CCommonTime& pTime )
{
...
/*************************************************************
 ...
 参    数:	pTime 输入的CXJTime对象
*************************************************************/
bool CCommonTime::operator<( CCommonTime& pTime )
{
    time_t tSour = mktime(&m_Time);
    time_t tDest = mktime(&pTime.m_Time);
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertSttpTime12ToTM( string &pTimeStr)
{
    pTimeStr.resize(12);
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertSttpTime14ToTM( string &pTimeStr)
{
    pTimeStr.resize(14);
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertSttpTime15ToTM( string &pTimeStr)
{
    pTimeStr.resize(15);
    m_Time.tm_year = atoi(pTimeStr.substr(0,2).c_str()); 
    if (m_Time.tm_year<70)
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertSttpTime19ToTM( string &pTimeStr )
{
    pTimeStr.resize(19);
    m_Time.tm_year = atoi(pTimeStr.substr(0,4).c_str())-1900; 
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertSttpTime26ToTM( string &pTimeStr )
{
    pTimeStr.resize(26);
    m_Time.tm_year = atoi(pTimeStr.substr(0,4).c_str())-1900; 
    m_Time.tm_mon = atoi(pTimeStr.substr(5,2).c_str())-1;
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertCP32Time2aToTM( string &pTimeStr )
{
    pTimeStr.resize(4);
    BYTE l = pTimeStr[0];
    BYTE h = pTimeStr[1];
...
/*************************************************************
 ...
 参    数:	pTimeStr 输入的时间字符串
*************************************************************/
void CCommonTime::ConvertCP56Time2aToTM( string &pTimeStr )
{
   pTimeStr.resize(7);
   BYTE l = pTimeStr[0];
   BYTE h = pTimeStr[1];
