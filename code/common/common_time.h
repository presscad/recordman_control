...
#ifndef Common_Time_h__
#define Common_Time_h__
...
/**
 * @defgroup 	CCommonTime	时间处理类
 * @{
 */
/**
 * @brief       该类用于系统中各种时间格式的处理及转换
 ...
 *    // 举例说明类如何使用
      1. 如果已有一个SYSTIME对象需要转换
            ...
            CCommonTime tTime(tSysTime);
            char chSttpTime26[27] = "";
            strncpy(chSttpTime26,tTime.GetTimeString(CCommonTime::STTP26Time).c_str,26);
            //转成其他格式的时间请输入对应的类型
      2. 如果已有一个time_t对象需要转换
            time_t tTTime;//如何设置值省去
            CCommonTime tTime(tTTime);
            char chSttpTime26[27] = "";
            strncpy(chSttpTime26,tTime.GetTimeString(CCommonTime::STTP26Time).c_str,26);
            //转成其他格式的时间请输入对应的类型
      3. 如果有一个时间字符串需要转换
            ...
            CCommonTime tTime;
            tTime.AssignTimeString(chTime,CCommonTime::STTP26Time);
            char ch103Time7[7] = "";
            strncpy(ch103Time7,tTime.GetTimeString(CCommonTime::CP56Time2a).c_str,7);
            //转成其他格式的时间请输入对应的类型
 * @endcode
 */
/** @} */ //OVER
...
public:
    ...
    CCommonTime(CCommonTime& pXJTime);
    /*
     ...
     *  @return		 
     */
    CCommonTime& operator= (CCommonTime& pTime);
    ...
    double DiffTime(CCommonTime& pTime);
     ...
     *  @return		bool 
     */
    bool operator< (CCommonTime& pTime);
    /*
