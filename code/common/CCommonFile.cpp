...
***************************************************************/
BOOL CCommonFile::Open(const char *name, UINT nOpenFlags)
{
	if(name == NULL)
	{
		memset(m_chError,0,FILE_ERROR_MAX_LEN);
...
***************************************************************/
UINT CCommonFile::Read(void * lpbuf, UINT nCount)
{
	UINT nRead = 0;
	if(pfile == NULL) 
	{
...
***************************************************************/
UINT CCommonFile::Write(const void * lpBuf,UINT nCount)
{
	UINT nwrite = 0;
...
/*************************************************************
 ...
 参    数：long lOff   偏移量
           UINT nFrom  偏移起始地址
***************************************************************/
BOOL CCommonFile::Seek(long lOff,UINT nFrom)
{
	int nFlag = 0;
	if(pfile == NULL) 
	{
